...
const Hero = () => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      // 1. Split the heading into characters for the cinematic reveal
      ...
      // Looping animation for the scroll indicator
      gsap.to(".scroll-line", {
        y: 20,
        opacity: 0,
        duration: 1.5,
  ...
  return (
    <section ref={sectionRef} className="relative w-full h-screen overflow-hidden bg-[#1A1A1A]">
      ...
      {/* 4K Background Image (Replaced Video) */}
      <img
      ...
      <div className="relative z-10 flex flex-col items-center justify-center w-full h-full text-center px-6">
        ...
        <button 
          ref={buttonRef}
          className="group relative overflow-hidden px-10 py-4 border border-[#EAAC7F]/40 hover:border-[#EAAC7F] transition-colors duration-500 rounded-none bg-black/20 backdrop-blur-sm"
        >
      ...
      <div 
        ...
        className="absolute bottom-10 left-1/2 -translate-x-1/2 z-10 flex flex-col items-center gap-3 opacity-0"
      >
        <span className="text-[#EAAC7F] text-[10px] uppercase tracking-[0.2em] writing-vertical">Scroll</span>
        <div className="w-[1px] h-12 bg-white/20 relative overflow-hidden">
          <div className="scroll-line w-full h-1/2 bg-[#EAAC7F] absolute top-0 left-0" />
        </div>
      </div>
    </section>
  );
